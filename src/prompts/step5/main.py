...
# === GeminiAI設定 ===
client = genai.Client(api_key=constants.STEP5_GEMINI_API_KEY)
...
def main():
  try:
    # コンテキスト取得
    print ('')
    with open(WorkPaths.get_prompts_step_prompt_file_path(constants.STEP5_WORK_DIR), "r", encoding="utf-8") as f:
      context = f.read()
    ...
    if step3_ai_response_dict is None:
      raise RuntimeError("Gemini response is empty")
    step3_ai_response_cleansed_json = json.dumps(step3_ai_response_dict["cleansed_context"])

    with open(_STEP4_AI_RESPONSE_FILE_PATH, "rb") as f:
    ...
    context = (
      f"{context}\n\n"
      + f"【投入データフォーマット】\n\nSTEP5を実行する際は、上記のガイドラインの下に、以下のフォーマットでデータを合体させて送信してください。\n\n"
      + f"【投入データ】\n\n"
      + f"【STEP2：構造化データ（JSON）】\n\n{step2_ai_response_data}\n\n"
      + f"【STEP3：ファクトチェック＆クレンジング済みコンテキスト】\n\n{step3_ai_response_cleansed_json}\n\n"
      + f"【STEP4：生成された最終HTML記事】\n\n{step4_ai_response_data}\n\n"
      + '上記の【STEP2（JSON）】および【STEP3（コンテキスト）】を絶対的な正解基準（マスターデータ）としてロックし、他の一切の先入観を排除してください。これらと【STEP4：最終HTML記事】を厳格に比較・監査し、ことわざのステルス融合、三人称客観視点の徹底、キラーフレーズの完全一致、コード完遂、ノイズゼロの基準に照らし合わせた品質保証レポート（JSONのみ）を出力してください。'
    )
    ...
    with open(WorkPaths.get_prompts_step_ai_response_file_path(youtube_id, constants.STEP5_AI_RESPONSE_FILE), "w", encoding="utf-8") as f:
